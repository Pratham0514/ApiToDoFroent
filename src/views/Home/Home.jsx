...
function Home() {
  ...
  {todos.map((todo) => (
    ...
    <div
      key={todo.id}
      className="relative bg-white rounded-2xl shadow-md hover:shadow-xl transition duration-300 overflow-hidden flex flex-col sm:flex-row"
    >

    {/* Priority bar */}
    <div className={`w-full sm:w-3 h-2 sm:h-auto ${
      todo.priority === "high"
      ? "bg-red-500"
    ...
    {/* Buttons */}
    <div className="flex sm:flex-col md:flex-row gap-2 p-4 sm:items-center">

      <button
        onClick={() => deleteTodo(todo.id)}
        className="bg-red-600 text-white px-3 py-1 rounded-xl shadow-md hover:bg-red-700 transition text-sm"
      >
        🗑 Delete
      </button>
      ...
      <Link
        to={`/edit/${todo.id}`}
        className="bg-indigo-600 text-white px-3 py-1 rounded-xl shadow-md hover:bg-indigo-700 transition text-sm"
      >
        📝 Edit
      </Link>
  ...
  <Link
    ...
    transition flex items-center gap-2 font-semibold text-sm md:text-base"
  >
    ➕ New Todo
  </Link>
